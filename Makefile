...
.PHONY: install lock lock-check lint format typecheck test build check validate validate-dataset audit no-dashes verify live-check check-links clean
...
#
# `validate` and `validate-dataset` run after `check`, and the order is the point: `check` proves site/ is what the
...
verify: lock-check lint no-dashes typecheck test check validate validate-dataset audit
	@echo "verify: ok"
