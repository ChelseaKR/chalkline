#!/usr/bin/env python3
"""Fail when the site GitHub Pages serves is not the site this checkout publishes.

`chalkline check` proves the committed `site/` is byte-for-byte what the code
produces from the vendored sources, and pages.yml runs it before every upload.
That grades the checkout. Nothing has ever graded the bytes a reader receives,
so a pages run that failed, never fired, or published an older commit would
leave every gate green while https://chelseakr.github.io/chalkline/ served a
different graph, a different coverage count, or a stale credential set, and
nothing in this repository could tell.

This is the check for the deployment. It runs `chalkline check` so the tree it
is about to compare is one the code still produces, fetches each published file
over HTTPS, and fails naming every byte-level difference.
...
LIVE_URL = "https://chelseakr.github.io/chalkline/"
...
REBUILD_COMMAND: tuple[str, ...] | None = ("uv", "run", "--locked", "chalkline", "check")
        ...
        try:
            connection.request(
                ...
                headers={
                    ...
                    "Pragma": "no-cache",
                    "User-Agent": "chalkline-live-integrity/1",
                },
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if differences:
        ...
        print(
            "\nRe-run publish-site to deploy the committed site/, or find out why the deployment is behind it.",
            file=sys.stderr,
        )
        return EXIT_DIFFERS
