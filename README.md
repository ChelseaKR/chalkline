...
| Code Quality | Applies | `make verify` is the gate and CI runs that exact target: `uv lock --check --offline`, ruff lint and format check, mypy over `src tests scripts`, pytest with a 97% coverage floor, and the build-output check. Floors are pinned in `pyproject.toml`: Python >= 3.12, ruff >= 0.16.2, mypy >= 2.3.0, complexity <= 10. Every invocation is `uv run --locked`, so a drifted lockfile fails rather than being silently repaired. |
