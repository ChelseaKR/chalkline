...
:mod:`chalkline.analytics` owns the loader. This module holds it to three things.

**The build.** No ID means no analytics on either page: no script, no reference to Google,
no opt-out control, and copy that says so. A malformed ID fails the build. With the committed
ID, both committed pages carry exactly one loader, the footer control and a link to the
privacy page, and the JSON files carry nothing.
...
from chalkline import analytics, site
...
ID: Final = "G-40XD8ZXCGR"
KEY: Final = "chalkline:analytics-opt-out"
...
PRODUCTION: Final = {"hostname": "chelseakr.github.io", "pathname": "/chalkline/"}

GUARDS: Final = {
    "host": 'if (w.location.hostname !== "chelseakr.github.io") return;',
    "path": 'if (w.location.pathname.indexOf("/chalkline/") !== 0) return;',
...
def test_no_id_means_no_analytics_on_either_page(
    empty: str | None, real_catalog: Catalog, real_attachments: dict[str, Attachment]
) -> None:
    pages = {
        "index": site.render(
            real_catalog, ctid_module.load_ledger(), real_attachments, ga4_id=empty
        ),
        "privacy": site.render_privacy(empty),
    }
...
def test_a_malformed_id_fails_the_build() -> None:
    for bad in ("UA-12345-1", "G-abc123", 'G-ABC"};alert(1);//', "G-", "40XD8ZXCGR"):
...
def test_the_data_files_carry_no_analytics() -> None:
    for name in ("credentials.jsonld", "coverage.json"):
        text = (SITE_DIR / name).read_text(encoding="utf-8")
        assert ID not in text, name
        assert "googletagmanager" not in text, name


def test_the_privacy_page_makes_no_root_relative_reference() -> None:
    """Same reason as the main page: /x resolves against the shared origin, not /chalkline/."""
    source = PRIVACY.read_text(encoding="utf-8")
    assert re.findall(r'(?:href|src|content)="(/(?!/)[^"]*)"', source) == []
    assert (
        '<link rel="canonical" href="https://chelseakr.github.io/chalkline/privacy.html">'
    ) in source
...
def test_the_opt_out_key_names_this_project() -> None:
    ...
    assert KEY.startswith("chalkline:")
...
def test_off_the_production_host_or_path_it_loads_nothing(tmp_path: Path) -> None:
    cases = [
        ...
        scenario("file", hostname="", pathname="/Users/x/site/index.html", protocol="file:"),
        scenario("other-host", hostname="example.com"),
        scenario("sibling-project", pathname="/cairn/"),
        scenario("origin-root", pathname="/"),
        scenario("prefix-lookalike", pathname="/chalkline-fork/"),
    ]
...
def test_an_unrelated_or_false_signal_still_loads(tmp_path: Path) -> None:
    ...
    cases = [
        ...
        scenario("deep-path", pathname="/chalkline/privacy.html"),
    ]
    for name, result in run(loader(INDEX), cases, tmp_path).items():
