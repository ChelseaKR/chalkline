...
``pyproject.toml`` declares ``version = "0.1.0"``. Nothing in this repository
has ever been tagged: ``git tag --list`` is empty here and on ``origin``. That
is not a defect on its own, because this is genuinely pre-release work. The
defect is publishing the number without saying so, and it is a portfolio-wide
one: measured on 2026-09-06, twenty public repositories declared a version in
``pyproject.toml`` that nothing had been tagged for. ``0.1.0`` looks exactly
like a version somebody could install, and a reader has no way to tell that
this one names no artifact.

It is the same shape as the defect this repository already gates in
``tests/test_documented_counts.py`` and ``tests/test_provenance.py``: a value
published where there is nothing behind it. So the rule here is the one those
files use, applied to the version. Nothing is transcribed. ``pyproject.toml``
is the single source, and every other copy of the number is compared against it
rather than trusted.

The rule sorts the declared version into one of two states.

*No tag exists.* Legitimate, and it passes, but only if the repository says so
where a reader arrives: the ``**Status:**`` line at the top of the README, or
the ``Release & Versioning`` row of the Standards table. The sentence has to
name the version ``pyproject.toml`` declares. Naming it is what stops the
disclosure going stale, since bumping to ``0.2.0`` and leaving a README that
explains ``0.1.0`` reads, to anyone who does not diff the two, exactly like a
current disclosure.

*Tags exist and none of them names the declared version.* A defect, and a
failure. The message names the declared version and the newest tag, because
"the version is wrong" without both numbers sends the reader back to a shell
to find out which two things disagree.

This repository is in the first state, so the second is exercised from
synthetic input by the negative controls below rather than waiting for a bad
day. A gate whose failing branch never runs is a gate nobody has evidence
about, which is how this portfolio has previously shipped a ``main()`` that
always returned 0 and an assertion only reachable when a value was redacted.
``release_claim_failure`` is therefore a pure function of the three things it
reads, and ``test_the_rule_reads_this_readme_rather_than_passing_regardless``
closes the remaining gap by sabotaging the real README and requiring the real
check to notice.
...
has to be run somewhere that could hold them. ``_why_tags_are_not_authoritative``
refuses to answer from a shallow clone or one configured ``--no-tags``: "no
tags found" from a checkout that was never given any is exactly the vacuous
pass this file exists to prevent. ``actions/checkout`` fetches no tags at its
default depth, so the job that runs this suite checks out with
``fetch-depth: 0``.
...
tag puts ``main`` into the failing state above, and accurately so, because for
that window the repository really does declare a version nothing was tagged for.
"""
...
RESTATEMENTS: tuple[tuple[Path, str], ...] = (
    ...
    (REPO_ROOT / "src" / "chalkline" / "__init__.py", r'^__version__ = "([^"]+)"'),
)
