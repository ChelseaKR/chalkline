...
This module is the one place that decision lives. ``chalkline.site`` asks it for the loader
that goes in each page's ``<head>`` and for the footer's privacy line, and both pages say
what a build actually ships because both are rendered from :data:`GA4_MEASUREMENT_ID`.
...
- Returns, loading nothing, unless the page is served from :data:`GA4_HOST` under
  ...
  guard: a ``file://`` or localhost preview, CI, or any other copy never loads GA and never
  sends a hit to the real property.
...
GA4_MEASUREMENT_ID: Final = "G-40XD8ZXCGR"
"""The chelseakr.github.io/chalkline web stream. Empty ("") means no analytics anywhere."""
...
GA4_OPT_OUT_KEY: Final = "chalkline:analytics-opt-out"
...
def footer_note(ga4_id: str | None) -> str:
    ...
    return (
        '<p class="privacy-note">This site counts visits with Google Analytics 4, advertising '
